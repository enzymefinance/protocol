...
import { encodeArgs, encodeFunctionData } from '../utils/common';
...
export function encodeZeroExV2AssetData(tokenAddress: string) {
...
export async function createUnsignedZeroExV2Order({
  ...
  const [
    ...
    await Promise.all([
      ...
      resolveAddress(maker),
      resolveAddress(makerAsset),
      resolveAddress(takerAsset),
    ])
...
export async function encodeZeroExV2Order(order: SignedZeroExV2Order) {
  return encodeArgs(
...
export async function zeroExV2TakeOrderArgs(
  order: SignedZeroExV2Order,
  takerAssetFillAmount: BigNumberish,
) {
  return encodeArgs(
    ['bytes', 'uint256'],
    [encodeZeroExV2Order(order), takerAssetFillAmount],
  );
}
