...
export async function seedAndThawEngine(provider: EthereumTestnetProvider, engine: Engine, amount: BigNumber) {
  await engine.payAmguInEther.value(amount).send();
  await warpEngine(provider, engine);
  await engine.thaw();
}

export async function warpEngine(provider: EthereumTestnetProvider, engine: Engine) {
