...
export type DeploymentHandler<
  ...
  TConfig = any,
  TOutput = any
> = (config: TConfig, deployment: TDeployment) => TOutput;

export type DeploymentHandlers<TConfig, TDeployment> = {
  [TKey in keyof TDeployment]: DeploymentHandler<
    TDeployment,
    TConfig,
    TDeployment[TKey]
  >;
...
function createRecursionAwareProxy<
  THandlers extends DeploymentHandlers<any, any>,
  TConfig
>(
  ...
  return new Proxy(state, {
    ...
    get: (target, prop, receiver) => {
      ...
      if (constructors.hasOwnProperty(prop) && !Reflect.has(target, prop)) {
        ...
        const proxy = createRecursionAwareProxy(
          constructors,
          config,
          state,
          path,
...
export function createDeploymentProxy<
  THandlers extends DeploymentHandlers<any, any>,
  TConfig
>(constructors: THandlers, config: TConfig) {
  const state = {} as DeploymentProxy<THandlers>;
  return createRecursionAwareProxy(constructors, config, state);
}

export async function resolveDeploymentProxy<
  THandlers extends DeploymentHandlers<any, any>
...
export function describeDeployment<TConfig, TOutput>(
  handlers: DeploymentHandlers<TConfig, TOutput>,
) {
