...
contract KyberPriceFeed is IPriceSource {
    ...
    address public immutable KYBER_NETWORK_PROXY;
    address public immutable PRICE_FEED_QUOTE_ASSET;
        ...
        )
    {
        ...
        if (_baseAsset == _quoteAsset) {
            return (10**uint256(ERC20(_quoteAsset).decimals()), true, timestamp_);
        }
        ...
        if (_quoteAsset != PRICE_FEED_QUOTE_ASSET) {
            rate_ = baseAssetPrice.mul(10**uint256(ERC20(_quoteAsset).decimals())).div(
                quoteAssetPrice
            );
        } else {
        ...
        returns (uint256 rate_, bool isValid_)
    {
        // Return early if assets are same
        if (_baseAsset == _quoteAsset) {
            return (10**uint256(ERC20(_quoteAsset).decimals()), true);
        }
    ...
    function hasValidPrice(address _asset) public override view returns (bool isValid_) {
    ...
    function __calcSrcQtyForExpectedRateLookup(address _srcAsset) private view returns (uint256) {
        ...
        return
            expectedRateWethQty.mul(10**uint256(ERC20(_srcAsset).decimals())).div(
                lastSrcAssetPrice
            );
    }
    ...
    function __priceIsSane(uint256 _priceFromKyber, uint256 _sanePrice)
        private
        view
        returns (bool)
    {
