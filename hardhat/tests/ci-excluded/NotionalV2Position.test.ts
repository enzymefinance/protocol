...
describe('init', () => {
  it('happy path', async () => {
    ...
    expect(receipt).toMatchInlineGasSnapshot(`462035`);
  });
});

describe('addCollateral', () => {
  it('works as expected (erc20)', async () => {
    const collateralAsset = new ITestStandardToken(fork.config.primitives.usdc, provider);
    ...
    expect(addCollateralReceipt).toMatchInlineGasSnapshot(`430953`);
  });

  it('works as expected (weth)', async () => {
    const collateralAsset = new ITestStandardToken(fork.config.weth, provider);
    ...
    // Check account balances
    expect(accountBalanceAfter.cashBalance_).toBeGtBigNumber(0);
    expect(addCollateralReceipt).toMatchInlineGasSnapshot(`330403`);
  });
});
...
xdescribe('lend', () => {
  ...
  it('works as expected (weth)', async () => {
    const underlyingAsset = new ITestStandardToken(fork.config.weth, provider);
    const underlyingAssetUnit = await getAssetUnit(underlyingAsset);
  ...
  it('works as expected (repaying debt)', async () => {
    const underlyingAsset = new ITestStandardToken(fork.config.primitives.dai, provider);
    const borrowedAsset = new ITestStandardToken(fork.config.primitives.usdc, provider);
...
xdescribe('borrow', () => {
  ...
  it('works as expected (erc20), posting fcash collateral', async () => {
    ...
    expect(borrowReceipt).toMatchInlineGasSnapshot(`701413`);
  });

  it('works as expected (weth), posting erc20 collateral', async () => {
    const collateralAsset = new ITestStandardToken(fork.config.primitives.dai, provider);
  ...
  it('works as expected (partially offsetting fCash loan)', async () => {
    ...
    expect(borrowReceipt).toMatchInlineGasSnapshot(`552359`);
  });
  ...
  it('works as expected (fully offsetting fCash loan)', async () => {
    ...
    expect(borrowReceipt).toMatchInlineGasSnapshot(`476415`);
  });
});
...
describe('redeem', () => {
  it('works as expected (partial redemption)', async () => {
    ...
    expect(redeemReceipt).toMatchInlineGasSnapshot(`381509`);
  });
  ...
  it('works as expected (full redemption)', async () => {
    ...
    expect(redeemReceipt).toMatchInlineGasSnapshot(`374886`);
  });
  ...
  it('works as expected (weth)', async () => {
    ...
    expect(redeemReceipt).toMatchInlineGasSnapshot(`337314`);
  });
});
