...
test.before(async () => {
  ...
  competition = await deployContract("competitions/Competition", Object.assign(opts, {gas: 6800000}), [deployed.MlnToken.address, version.address, accounts[5], blockchainTime, blockchainTime + competitionDuration, 10 ** 17, 10 ** 22, 10], () => {}, true);
...
test.serial('Competition claimReward transfers shares to the registrant', async t => {
  // Shares of the registrant fund
  const pre = await getAllBalances(deployed, accounts, fund);
  const managerPreShares = await fund.instance.balanceOf.call({}, [manager]);
  const competitionPreShares = await fund.instance.balanceOf.call({}, [competition.address]);
  ...
  const competitionPostShares = await fund.instance.balanceOf.call({}, [competition.address]);
  ...
  t.deepEqual(competitionPostShares, competitionPreShares.sub(expectedReward));
