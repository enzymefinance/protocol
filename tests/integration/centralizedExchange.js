...
test.serial("Manager makes an order through centralized exchange adapter", async t => {
  ...
  t.deepEqual(post.exchangeOwner.MlnToken, pre.exchangeOwner.MlnToken.add(trade1.sellQuantity));
...
test.serial("Manager cancels an order from the fund",
  async t => {
    ...
    t.deepEqual(post.exchangeOwner.MlnToken, pre.exchangeOwner.MlnToken.minus(trade1.sellQuantity));
    t.deepEqual(post.investor.MlnToken, pre.investor.MlnToken);
    t.deepEqual(post.investor.EthToken, pre.investor.EthToken);
    t.deepEqual(post.investor.ether, pre.investor.ether);
    t.deepEqual(post.manager.EthToken, pre.manager.EthToken);
