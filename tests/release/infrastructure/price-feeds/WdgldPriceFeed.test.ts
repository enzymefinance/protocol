...
import { calcGtr, calcGtrConstant } from '@melonproject/protocol/src/utils/price-feeds/wdgld';
...
describe('constructor', () => {
  it('sets initial storage vars', async () => {
    ...
    const storedGtrConstant = await wdgldPriceFeed.getGtrConstant();
    const storedWdgldGenesisTimestamp = await wdgldPriceFeed.getWdgldGenesisTimestamp();
    ...
    expect(storedGtrConstant).toEqBigNumber(await calcGtrConstant());
    expect(storedWdgldGenesisTimestamp).toEqBigNumber(1568700000);
  });
});
