...
async function getFundComponents(hubAddress) {
  ...
  const participationAddress = await components.hub.methods
    .participation()
    .call();
  ...
  const policyManagerAddress = await components.hub.methods
    .policyManager()
    .call();
  components.participation = await retrieveContract(
    'fund/participation/Participation',
    participationAddress,
  );
  components.shares = await retrieveContract(
    'fund/shares/Shares',
    sharesAddress,
  );
  components.trading = await retrieveContract(
    'fund/trading/Trading',
    tradingAddress,
  );
  components.policyManager = await retrieveContract(
    'fund/policies/PolicyManager',
  ...
  components = Object.assign(components, {
    accounting: await retrieveContract(
      'fund/accounting/Accounting',
      routes.accounting,
    ),
    feeManager: await retrieveContract(
      'fund/fees/FeeManager',
      routes.feeManager,
    ),
    participation: await retrieveContract(
      'fund/participation/Participation',
      routes.participation,
    ),
    policyManager: await retrieveContract(
      'fund/policies/PolicyManager',
...
async function deployEnvironment(environment) {
  ...
  if (config === undefined) {
    ...
    if (nodeNetId !== config.networkId && config.networkId !== '*') {
      throw new Error(
        `Network ID of node (${nodeNetId}) did not match ID in config "${environment}" (${
          config.networkId
        })`,
      );
    }
  ...
  if (environment === 'kovan' || environment === 'kovanCompetition') {
    ...
    deployed.CanonicalPriceFeed = await retrieveContract(
      'pricefeeds/CanonicalPriceFeed',
      previous.CanonicalPriceFeed,
    );
    ...
    deployed.MatchingMarket = await retrieveContract(
      'exchange/thirdparty/MatchingMarket',
      previous.MatchingMarket,
    );
    deployed.MatchingMarketAdapter = await retrieveContract(
      'exchange/adapter/MatchingMarketAdapter',
      previous.MatchingMarketAdapter,
    );

    deployed.ZeroExTokenTransferProxy = await retrieveContract(
      'exchange/thirdparty/0x/TokenTransferProxy',
      previous.ZeroExTokenTransferProxy,
    );
    deployed.ZeroExExchange = await retrieveContract(
      'exchange/thirdparty/0x/Exchange',
      previous.ZeroExExchange,
    );
    deployed.ZeroExV1Adapter = await retrieveContract(
      'exchange/adapter/ZeroExV1Adapter',
      previous.ZeroExV1Adapter,
    );
    ...
    deployed.NoCompliance = await deployContract(
    ...
    deployed.NoComplianceCompetition = await deployContract(
      'compliance/NoComplianceCompetition',
      opts,
      [],
    );
    deployed.CompetitionCompliance = await deployContract(
    ...
    const complianceAddress =
      environment === 'kovan'
        ? deployed.NoComplianceCompetition.options.address
        : deployed.CompetitionCompliance.options.address;
    ...
    deployed.Competition = await deployContract(
      ...
      [
        mlnAddr,
        chfAddress,
        deployed.Version.options.address,
        deploymentAddress,
        blockchainTime,
        blockchainTime + 8640000,
        20 * 10 ** 18,
        10 ** 24,
        1000,
        false,
    ...
    deployed.Governance = await retrieveContract(
      'system/Governance',
    ...
    deployed.CanonicalPriceFeed = await retrieveContract(
      'pricefeeds/CanonicalPriceFeed',
      '0x4e224B5500FB9D6456069039D27c1E989429EAb7',
    );
    ...
    deployed.BugBountyCompliance = await retrieveContract(
      'compliance/BugBountyCompliance',
      '0xD42316be0E813104096ab537FeE2fe0f5076bB2F',
    );
    deployed.Version = await retrieveContract(
      'version/Version',
      '0x569D8c4408005AD48C7fA439BE926476ec0e96b4',
    );
    ...
    await deployed.TestingPriceFeed.methods
      .setDecimals(deployed.MlnToken.options.address, 18)
      .send(clone(opts));
    await deployed.TestingPriceFeed.methods
    ...
    await deployed.MatchingMarket.methods
      .setMatchingEnabled(false)
      .send(clone(opts));
    deployed.MatchingMarket.methods
    ...
    deployed.CanonicalPriceFeed = await deployContract(
      'pricefeeds/CanonicalPriceFeed',
      opts,
      [
        deployed.MlnToken.options.address,
    ...
    deployed.StakingPriceFeed = await createStakingFeed(
      { ...opts },
      deployed.CanonicalPriceFeed,
    );
    await deployed.MlnToken.methods
      .approve(
        deployed.StakingPriceFeed.options.address,
        config.protocol.staking.minimumAmount,
      )
      .send({ ...opts });
    await deployed.StakingPriceFeed.methods
      .depositStake(
        config.protocol.staking.minimumAmount,
    ...
    deployed.SimpleMarket = await deployContract(
      'exchange/thirdparty/SimpleMarket',
      opts,
    );
    deployed.SimpleAdapter = await deployContract(
      'exchange/adapter/SimpleAdapter',
      opts,
    );
    deployed.MatchingMarket = await deployContract(
      'exchange/thirdparty/MatchingMarket',
    ...
    deployed.RMMakeOrders = await deployContract('riskmgmt/RMMakeOrders', opts);
    deployed.CentralizedAdapter = await deployContract(
      'exchange/adapter/CentralizedAdapter',
      opts,
    );
    deployed.CompetitionCompliance = await deployContract(
      'compliance/CompetitionCompliance',
    ...
    await deployed.CompetitionCompliance.methods
      .changeCompetitionAddress(deployed.Competition.options.address)
      .send(opts);
    await deployed.Competition.methods
      .batchAddToWhitelist(new BigNumber(10 ** 25), [
        accounts[0],
        accounts[1],
        accounts[2],
      ])
    ...
    await Promise.all(
      pairsToWhitelist.map(async pair => {
        await deployed.MatchingMarket.methods
          .addTokenPairWhitelist(pair[0], pair[1])
          .send(opts);
      }),
    );
...
if (require.main === module) {
  ...
  if (environment === undefined) {
    throw new Error(
      `Please specify an environment using the environment variable CHAIN_ENV`,
    );
  } else {
    deployEnvironment(environment)
      .then(deployedContracts =>
        writeToAddressBook(deployedContracts, environment),
      )
