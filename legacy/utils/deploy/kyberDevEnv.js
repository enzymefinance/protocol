...
async function setupKyberDevEnv(preKyberDeployed, accounts) { 
  ...
  deployed.KGTToken = await deployContract("exchanges/thirdparty/kyber/TestToken", opts, ["KGT", "KGT", 18]);
  ...
  await mlnToken.methods.transfer(deployed.KyberReserve.options.address, new BigNumber(10 ** 23).toFixed()).send();
  const mlnPrice = new BigNumber(10 ** 18); // Arbritrary for now
  const ethersPerToken = mlnPrice.toFixed();
  const tokensPerEther = new BigNumber(precisionUnits).mul(precisionUnits).div(ethersPerToken).toFixed(0);
  ...
  await deployed.KyberWhiteList.methods.setCategoryCap(0, new BigNumber(10 ** 28).toFixed()).send();
  ...
  await web3.eth.sendTransaction({to: deployed.KyberReserve.options.address, from: accounts[0], value: new BigNumber(10 ** 24)});
  await deployed.KyberReserve.methods.setContracts(deployed.KyberNetwork.options.address, deployed.ConversionRates.options.address, "0x0000000000000000000000000000000000000000").send();
  ...
  await eurToken.methods.transfer(deployed.KyberReserve.options.address, new BigNumber(10 ** 23).toFixed()).send();
  const eurPrice = new BigNumber(10 ** 18); // Arbritrary for now
  const ethersPerEurToken = eurPrice.toFixed();
  const eurTokensPerEther = new BigNumber(precisionUnits).mul(precisionUnits).div(ethersPerEurToken).toFixed(0);
