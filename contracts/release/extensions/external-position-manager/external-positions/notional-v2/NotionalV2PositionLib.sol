...
import "../../../../../persistent/external-positions/notional-v2/NotionalV2PositionLibBase1.sol";
    ...
    AssetHelpers
{
    ...
    function __actionBorrow(bytes memory _actionArgs) private {
        ...
        if (!isBorrowerPosition()) {
            isBorrower = true;

            emit BorrowingExternalPositionActivated();
        }
    }
        ...
        returns (address[] memory assets_, uint256[] memory amounts_)
    {
        if (isBorrowerPosition()) {
            ...
            if (assets_.length == 0) {
                ...
                emit BorrowingExternalPositionDeactivated();
            }
        }
    ...
    /// @notice Checks whether the external position is borrowing
    function isBorrowerPosition() public view returns (bool isBorrower_) {
        return isBorrower;
    }
}
