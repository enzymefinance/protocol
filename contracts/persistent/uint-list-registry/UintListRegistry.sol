...
contract UintListRegistry is IUintListRegistry {
    ...
    function addToList(uint256 _id, uint256[] calldata _items) external override onlyListOwner(_id) {
    ...
    function createList(address _owner, UpdateType _updateType, uint256[] calldata _initialItems)
        external
        override
        returns (uint256 id_)
    {
    ...
    function removeFromList(uint256 _id, uint256[] calldata _items) external override onlyListOwner(_id) {
        UpdateType updateType = getListUpdateType(_id);
        require(
    ...
    function areAllInList(uint256 _id, uint256[] memory _items) external view override returns (bool areAllInList_) {
        for (uint256 i; i < _items.length; i++) {
            if (!isInList(_id, _items[i])) {
                return false;
            }
    ...
    function isInAllLists(uint256[] memory _ids, uint256 _item) public view override returns (bool isInAllLists_) {
        for (uint256 i; i < _ids.length; i++) {
    ...
    function isInList(uint256 _id, uint256 _item) public view override returns (bool isInList_) {
