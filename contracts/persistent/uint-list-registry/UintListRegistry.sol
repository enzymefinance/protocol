...
contract UintListRegistry {
    ...
    function isInAllLists(uint256[] memory _ids, uint256 _item) public view returns (bool isInAllLists_) {
        for (uint256 i; i < _ids.length; i++) {
    ...
    function isInSomeOfLists(uint256[] memory _ids, uint256 _item) public view returns (bool isInSomeOfLists_) {
        for (uint256 i; i < _ids.length; i++) {
            if (isInList(_ids[i], _item)) {
                return true;
            }
