...
async function getPricesFromAPI(tokenArray) {
  ...
  const prices = {};
  for (const i of Object.keys(tokenArray)) {
    const inversePrice = new BigNumber(1).div(new BigNumber(queryResult[tokenArray[i].symbol])).toNumber().toFixed(15);
    const sellPrice = new BigNumber(inversePrice).div(10 ** (tokenArray[i].decimals - quoteDecimals)).times(10 ** tokenArray[i].decimals);
    const buyPrice =  new BigNumber(new BigNumber(10 ** quoteDecimals).mul(10 ** tokenArray[i].decimals).div(sellPrice).toFixed(0));
